...
bool ColorShader::Initialize(OpenGLRenderer* _renderer, HWND _hwnd)
{
	bool result;
	...
	result = _initializeShader("vs_color.glsl", "ps_color.glsl", _hwnd);
	if (!result)
	{
		return false;
	}
...
bool ColorShader::_initializeShader(char* _vsFilename, char* _psFilename, HWND _hwnd)
{
	...
	m_Renderer->glBindAttribLocation(m_shaderProgram, 1, "inputColor");
...
char* ColorShader::_loadShaderSourceFile(char* _filename)
{
	std::ifstream fin;
	int fileSize;
	char input;
...
void ColorShader::_outputShaderErrorMessage(HWND _hwnd, unsigned int _shaderId, char* _shaderFilename)
{
	int logSize, i;
	char* infoLog;
	std::ofstream fout;
...
void ColorShader::_outputLinkerErrorMessage(HWND _hwnd, unsigned int _programId)
{
	int logSize, i;
	char* infoLog;
	std::ofstream fout;
...
void ColorShader::_shutdownShader()
{
...
bool ColorShader::SetShaderParameters(float* _worldMatrix, float* _viewMatrix, float* _projectionMatrix)
{
